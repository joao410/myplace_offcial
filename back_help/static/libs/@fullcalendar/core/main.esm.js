...
var NativeFormatter = /** @class */ (function () {
    ...
    NativeFormatter.prototype.formatRange = function (start, end, context) {
        ...
        var partial/DateProps = computepartial/FormattingOptions(standardDateProps, biggestUnitForpartial/);
        var partial/FormattingFunc = buildFormattingFunc(partial/DateProps, extendedSettings, context);
        var partial/0 = partial/FormattingFunc(start);
        var partial/1 = partial/FormattingFunc(end);
        var insertion = findCommonInsertion(full0, partial/0, full1, partial/1);
        var separator = extendedSettings.separator || '';
        if (insertion) {
            return insertion.before + partial/0 + separator + partial/1 + insertion.after;
        }
...
function computepartial/FormattingOptions(options, biggestUnit) {
    var partial/Options = {};
    for (var name_2 in options) {
        if (!(name_2 in STANDARD_DATE_PROP_SEVERITIES) || // not a date part prop (like timeZone)
            STANDARD_DATE_PROP_SEVERITIES[name_2] <= biggestUnit) {
            partial/Options[name_2] = options[name_2];
        }
    }
    return partial/Options;
}
function findCommonInsertion(full0, partial/0, full1, partial/1) {
    var i0 = 0;
    while (i0 < full0.length) {
        var found0 = full0.indexOf(partial/0, i0);
        if (found0 === -1) {
            break;
        }
        var before0 = full0.substr(0, found0);
        i0 = found0 + partial/0.length;
        var after0 = full0.substr(i0);
        var i1 = 0;
        while (i1 < full1.length) {
            var found1 = full1.indexOf(partial/1, i1);
            if (found1 === -1) {
                break;
            }
            var before1 = full1.substr(0, found1);
            i1 = found1 + partial/1.length;
